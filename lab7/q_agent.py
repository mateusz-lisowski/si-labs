...
class QAgent(Agent):
    ...
    def choose_action(self, state: State) -> Action:
        ...
        if np.random.uniform(0, 1) < self.epsilon:
            ...
            # Wybierz akcję na podstawie wartości Q
    ...
    def learn(self, state: State, action: Action, reward: float, new_state: State, done: bool) -> None:
        ...
        q_next = np.max(self.q_table[new_state]) if not done else 0  # Jeśli jesteśmy w stanie końcowym, to wartość Q dla kolejnego stanu wynosi 0
